---
title: Coastal Keepsake Shadow Box I
category: coastal-keepsakes
descriptionParagraphs:
  - Handcrafted from a thoughtfully curated collection of natural seashells,
    this framed coastal keepsake celebrates the quiet beauty of the shoreline.
    Layered by hand to create depth and texture, each shell retains its natural
    character, making every piece one of a kind. Finished in a soft sage shadow
    box frame, it brings a subtle coastal elegance to shelves, consoles,
    bookcases, or gallery walls.
  - Whether displayed on its own or grouped with other Bartram Studio creations,
    this piece offers a timeless reminder of the sea and the memories it
    inspires.
  - |-
    **Details**

    * Dimensions: Approximately 5” × 5”
    * Frame Color: Sage Green
    * Materials: Natural seashells, shadow box frame
    * Handcrafted: Yes
    * One of a Kind: Yes
price: 55
status: available
quantity: 1
featured: true
customOrderAvailable: false
images:
  - /images/products/cpk-002-02-hero.png
  - /images/products/cpk-002-02-angle.png
  - /images/products/cpk-002-02-lifestyle.png
---
