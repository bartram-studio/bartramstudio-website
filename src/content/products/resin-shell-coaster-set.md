---
category: coastal-keepsakes
descriptionParagraphs:
  - Inspired by the quiet beauty of the shoreline, this handcrafted shadow box
    transforms natural seashells into a blooming coastal sculpture. Carefully
    layered shells create the shape of a delicate flower, while a sculptural
    seashell centerpiece and thoughtfully placed natural shell accents add
    depth, texture, and dimension within the frame.
  - Designed as a timeless piece of coastal wall art, this one-of-a-kind
    creation brings warmth, texture, and the calming spirit of the ocean to
    shelves, gallery walls, entryways, bedrooms, or beach cottages. Its compact
    size makes it easy to display on a wall or style on a bookshelf, where it
    becomes a beautiful reminder of nature’s quiet elegance.
  - Each Bartram Studio piece is individually handcrafted using genuine
    seashells. Natural variations in color, shape, and texture ensure that every
    creation is truly one of a kind.
  - |-
    Details

    * Handcrafted using genuine seashells
    * Three-dimensional floral shell sculpture
    * Shadow box frame ready to display
    * One-of-a-kind artisan creation
    * Compact artwork ideal for shelves or gallery walls
    * Beautiful for coastal, cottage, and beach-inspired homes
    * Handmade in Florida by Bartram Studio
price: 55
quantity: 1
slug: resin-shell-coaster-set
title: Sea Blossom Shadow Box
status: available
featured: true
customOrderAvailable: true
images:
  - /images/products/cpk-001-01-hero.png
  - /images/products/cpk-001-01-angle.png
  - /images/products/cpk-001-01-detail.png
  - /images/products/cpk-001-01-hero.png
description: "A set of four round resin coasters each uniquely embedded with
  whole shells, shell fragments, and iridescent glitter in ocean-inspired hues.
  No two coasters are identical — the shells shift and sparkle depending on the
  light. Sealed with a durable UV-resistant top coat for everyday use. Arrive in
  a gift-ready box tied with natural twine. Diameter: 4 inches each."
---
